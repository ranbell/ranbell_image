...
from app.muse_refine import assemble, service
...
async def test_the_model_is_dropped_before_the_board_is_queued(rig):
    ...
    assert rig.log == ["rebuild", "unload", "submit:muse_refine_board"]
...
async def test_the_model_is_dropped_before_the_shoot_is_queued(rig):
    ...
    assert rig.log[-2:] == ["unload", "submit:muse_refine_shoot"]
...
async def test_the_switch_is_muses_switch(rig):
    ...
    assert rig.log == ["rebuild", "submit:muse_refine_board"]
