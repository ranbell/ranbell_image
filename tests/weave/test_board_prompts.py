...
def test_sheet_is_one_multi_view_image():
    """The mood board is a character sheet, not a fifth single portrait."""
    out = compile_board_slot(_preset_session("shy_bookworm"), "sheet")
    text = out["positive"]
...
def test_sheet_varies_the_four_frames_for_every_preset():
    for preset in load_seed_presets():
        session = new_session_payload()
        session["character"].update(preset_to_character(preset))
        lines = [
            line for line in compile_board_slot(session, "sheet")["positive"].splitlines()
            if line.startswith(" - ")
        ]
...
def test_sheet_does_not_repeat_the_hobby_as_the_active_frame():
    # Her hobby already is running; the active frame must show something else.
    out = compile_board_slot(_preset_session("morning_runner"), "sheet")
...
def test_sheet_takes_the_food_word_not_the_sentence():
    out = compile_board_slot(_preset_session("shy_bookworm"), "sheet")
...
def test_board_is_portrait_plus_sheet():
    session = new_session_payload()
    ...
    sheet = compile_board_slot(session, "sheet")
    ...
    # The sheet carries the full body and the throughline prop that the retired
    # `full` / `prop` slots used to confirm separately.
    assert "full_body" in sheet["positive"]
    assert "holding cloth_bookmark" in sheet["positive"]
