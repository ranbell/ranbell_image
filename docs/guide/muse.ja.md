...
**キャンバスが正、ただし Action と Description は別。** 「絵から読んだものが composed に勝つ」は Outfit や Place では正しい — 512px の下書きが確実に報告できる事実だから。動詞では逆になる。安い下書きは特定の動作をほとんど描かないので、パン屋のお題で composed された `kneading_dough` に対し下書きは `sitting, eating` を返し、harvest が Action を丸ごと上書きして、完成プロンプトのキャラはパンを焼かずに食べていた。**下書きが描けなかったからこそ、本番で言い直す必要がある。** この2面だけ composed が先頭を取る。ただし上限の半分まで — 上書きが問題だったのであって、下書きの観察自体は役に立つ。
