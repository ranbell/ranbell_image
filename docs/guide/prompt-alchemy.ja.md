...
画像のウェイトスライダーを上げると、プロンプトへの影響が 2 つの経路で現れます。

1. **タグ予算** — そのウェイトに比例して固有タグの注入量が増える
2. **矛盾解決の優先度** — 2 枚の画像が相反する属性（例: `blonde_hair` vs `purple_hair`）を持つとき、高ウェイト画像のタグが勝ち、低ウェイト側のタグは除去される

つまりスライダーを 70% にすると、その画像のニュアンスが「ヒント」として加わるだけでなく、低ウェイト画像との矛盾を積極的に上書きします。
...
`「テキストボードに"Good!"って手に持っている」` や `"add the text 'RANBELL'"` のように指示すると、引用符で囲まれた文字列を自動抽出し、Anima モデル形式でプロンプト末尾に追記します。**VLM を介さないので文字が変形・誤解釈されるリスクがありません。**

```
指示: 「テキストボードに"Good!"って手に持っている」
         ↓ 引用文字列を自動抽出
最終プロンプト: 1girl, holding_sign, ..., text "Good!"

指示: 'Add the text "RANBELL"'
         ↓
最終プロンプト: 1girl, ..., text "RANBELL"
```

`text "X"` タグはポジティブプロンプトの末尾に付加されます。Anima および対応モデルが画像内に文字をそのまま描画します。
...
| 画像に文字を入れたい | 指示の中で文字を引用符で囲む（例: `"Good!"` ） |
