...
召喚は「種を無から生み出す」パネルです。あなたが与えるのは雰囲気・色・ざっくりしたイメージだけ。6つのスピリットがそれぞれの哲学で解釈を行い、最大6通りの種を並列で生成します。どれかを採用し、本生成へ送るか、もう一度振り直すか——選択はあなたの手に委ねられます。
...
| **愁** | Poet — Sorrow | シネマティックな哀愁に全振り。入力がどんなに明るくても、フィルムグレイン・くすみカラー・郷愁の光で包み込む。**デフォルト OFF（opt-in）** | 金（適合度 ≥ 85%） |

> **フレームの色について**  
> 映・漂・瞰・愁 は「意図に近い = 良い結果」なので適合度が高いほど金フレームで祝われます。  
> 逆・奔 は「意図から離れること = 成功」なので、適合度が低いほど黒曜石フレームで報われます。

> **愁 (Poet) について**  
> 愁はデフォルトで OFF になっています。スピリット選択セクションで手動でONにすると召喚に参加します。danbooru_tags には常に `film_grain, depth_of_field, bokeh, muted_colors, analog_film, vintage_aesthetic` が注入されます。
...
6つのスピリットを個別にオン/オフできます。1つ以上選択が必要です。特定のスピリットだけ比較したいときに絞り込めます。**愁 (Poet)** はデフォルト OFF です。シネマティックな哀愁を求めるときだけ有効にしてください。

---

### 共鳴モード（Echoes of Resonance）

カメラワークセクションの下に **🌟 Echoes of Resonance** トグルがあります。

このモードを ON にすると、4★以上に評価した画像群の埋め込みベクトル重心を計算し、あなたの美的感覚に近い Danbooru タグ上位20件をスピリットへの「character_hints」として自動注入します。使い込むほど、召喚結果があなたの好みに静かに引き寄せられます。

| 表示 | 意味 |
|---|---|
| 🌟 トグル（グレー） | 共鳴モード OFF |
| 🌟 トグル（紫） | 共鳴モード ON — 共鳴タグ上位5件が表示される |
| 「Rate 4★+ images to activate」 | 4★以上の評価済み画像がまだない（サイレントフォールバック） |

> **仕組み**: star_rating ≥ 4 の画像の embedding を star4×1.0、star5×2.0 の重みで合成し L2 正規化。その重心ベクトルから WD14 vocab を意味検索します。あなたが★4以上に評価した画像が多いほど、重心の精度が上がります。
...
| 映像的な郷愁・フィルムテイストがほしい | **愁 Poet** を有効にする |
| 自分の美的センスに染めていきたい | **共鳴モード（Echoes of Resonance）**を ON にする |
| 比較のために全パターンを見たい | 全スピリットを有効にする（愁は別途 ON が必要） |
