...
参照画像を 1〜6 枚選び、影響度ウェイトを設定して短い指示を書くだけで、VLM が意図通りにプロンプトを生成します。

**例：** キャラクター画像を 2 枚ピン留めして 70% / 30% に設定し、*「ウサ耳をつけてサマードレスにして」* と書くだけ。WD14 がウェイトに比例して各画像から視覚的な語彙を抽出し、矛盾（例: 髪色）を高ウェイト画像優先で解決して、Ollama が正確なプロンプトを合成します。
...
| **danbooru** | SD 系タグ学習モデル | カンマ区切りタグのみ |
| **detailed** | 構造化プロンプト向け | 8セクションの詳細記述 |

- **ウェイト対応 WD14 注入** — 全スタイルでタグ予算と矛盾解決がウェイトを反映
...
5つのスピリットが、あなたの雰囲気・色・ざっくりしたイメージを受け取り、それぞれの哲学で5通りの種（プロンプト + 画像）を**並列で**生み出します。どれかを採用して本生成へ送るか、再振りするか——選択はあなたの手に。

<!-- スクリーンショット: Invoke パネル（スピリットカード群） -->
![Invoke パネル](docs/screenshots/JA_invoke_panel.png)

| 漢字 | 英名 | 役割 |
|:----:|------|------|
| **映** | Mirror — Faithful | 意図を忠実に実現。軸の中心線から一切ぶれない |
| **逆** | Counter — Rebel | 欲望の「影」を見せる。1軸だけ反転して対比を生む |
| **漂** | Wander — Stranger | 珍しいゲスト要素を自然に織り込む |
| **奔** | Surge — Lunatic | 不可能を受け入れる。低共起タグで予測不能な結果を追求 |
| **瞰** | Vantage — Oracle | 完全な創作の自由。印象に残る結果を最優先 |

- **ライトモード**: 絵文字 39 種・4軸ムードスライダー（暖冷/静動/密疎/具抽）・カラーパレット・人物指定
- **プロモード**: プロンプト直接編集・お題→タグ変換・スピリット別シード指定
- **リアルタイム進行**: SSE ストリーミングでプロンプト生成 → 画像生成 → 評価を即時反映
- **採用 / 再振り / 本生成へ**: 気に入った種をコレクションに取り込むか、もう一度振り直すか選択

> 📖 [クリエイターガイド — 召喚](docs/guide/invoke.ja.md)
