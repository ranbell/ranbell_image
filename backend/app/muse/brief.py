...
gone. The brief *is* the mechanism: it is re-injected on every LLM call, and each
system prompt says the hair, the eyes, the figure and the clothing may not change.
Twelve renders across two themes held identity without a single reinsertion.
...
contamination: across every prompt of the validation runs, not one reference
...
REFERENCE_HEADER = (
    "** This is her's backgrounds.  For reference. "
    'Use for imagining "What she do in this situation". '
    "Do not directly include there keywords. **"
)
...
def build(character: dict[str, Any], theme: str, style: str) -> str:
    ...
    head = [f"Style: {style.strip()}" if style.strip() else "",
    ...
    tastes = [
        _line("favorite: ", [str(t) for t in (personality.get("likes") or [])]),
        _line("hate : ", [str(t) for t in (personality.get("dislikes") or [])]),
        _line("favorite color:", [str(c) for c in (character.get("palette") or [])], ", "),
        _line("favorite accesory: ", [str(character.get("signature_prop") or "")]),
...
def with_tags(brief: str, tags: str) -> str:
    """Stage B's input: the brief, then what WD14 actually read off the draft.

    Stage A's prose is deliberately *not* carried forward. The tags replace it —
    that substitution is the whole idea, because the tags describe the picture
    that exists rather than the one that was asked for.
    """
    return f"{brief},{tags}" if tags.strip() else brief
