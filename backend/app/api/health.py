...
async def detailed_health(request: Request):
    ...
    async def check_qdrant():
        try:
            ...
            # **What is still being built (2026-09-21).** Payload indexes are asked
            # for with `wait=False` so a first boot after an update does not stall
            # on them; until they land, a filtered search is a scan rather than a
            # lookup. The screen says so instead of looking broken.
            indexes = await db.index_progress()
            return {
                "ok": True,
                "doc_count": count,
