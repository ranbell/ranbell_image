...
def shoot_log_lines(chat: list) -> list[dict]:
    """The conversation of one shoot, in the shape the diary screen reads.

    **Her lines are stored as `assistant`.** This used to keep only
    `user` / `muse` / `system` — `muse` being classic's word for the same thing —
    so everything she and the crew said was dropped and the log came back as the
    Showrunner's typing plus a few studio notices. The screen styles her bubbles
    off `muse`, so the name is translated here rather than changing what a session
    stores.

    `_append_chat` puts everything but role/name/text under `meta`, so `kind` was
    read from a key that is never there — which is why a heckle never looked any
    different from a line she meant.
    """
    out: list[dict] = []
    for msg in chat:
        ...
        if not text or role not in ("user", "assistant", "muse", "system"):
            continue
        if role == "system" and _MACHINE_LINE_RE.match(text):
            continue
        meta = msg.get("meta") if isinstance(msg.get("meta"), dict) else {}
        out.append({
            "role": "muse" if role == "assistant" else role,
            "name": str(msg.get("name") or ""),
            "muse_id": str(meta.get("muse_id") or msg.get("muse_id") or ""),
            "kind": str(meta.get("kind") or msg.get("kind") or ""),
            "text": text,
        })
    return out
...
async def read_diary_shoot_log(character_id: str, diary_id: str, request: Request):
    ...
    lines = shoot_log_lines(session.get("chat") or [])
    out["available"] = True
    out["lines"] = lines
