...
class FakeQdrant:
    ...
    async def create_payload_index(
        self, collection_name, field_name, field_schema, *, wait=True, **kw,
    ):
        # `wait` is what the real client takes — the app asks for indexes with
        # `wait=False` so a first boot does not stall while they build.
